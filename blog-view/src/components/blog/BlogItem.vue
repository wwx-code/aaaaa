				...
				<div class="ui grid m-margin-lr">
					...
					<div class="row m-padded-tb-no">
						<div class="column m-padding-left-no">
							<router-link :to="`/tag/${tag.tagName}`" class="ui tag label m-text-500 m-margin-small" :style="{background:tag.color,color:'#fff'}" v-for="(tag,index) in item.tags" :key="index">{{ tag.tagName }}</router-link>
						</div>
					</div>
				</div>
			</div>
